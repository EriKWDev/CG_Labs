                           ...
                           bool generate_mipmap)
{
	...
	typedef struct name_kind_t { std::string const& name; GLuint kind; } name_kind_t;
	name_kind_t data_in_order[6] = {
		name_kind_t { .name = posx, .kind = GL_TEXTURE_CUBE_MAP_POSITIVE_X },
		name_kind_t { .name = negx, .kind = GL_TEXTURE_CUBE_MAP_NEGATIVE_X },

		name_kind_t { .name = posy, .kind = GL_TEXTURE_CUBE_MAP_POSITIVE_Y },
		name_kind_t { .name = negy, .kind = GL_TEXTURE_CUBE_MAP_NEGATIVE_Y },

		name_kind_t { .name = negz, .kind = GL_TEXTURE_CUBE_MAP_NEGATIVE_Z },
		name_kind_t { .name = posz, .kind = GL_TEXTURE_CUBE_MAP_POSITIVE_Z },
	};

	name_kind_t* current = data_in_order;
	for(int i = 0; i < 6; i++) {
		std::uint32_t width, height;
		auto data = getTextureData(current->name, width, height, false);
		if (data.empty()) {
			glDeleteTextures(1, &texture);
			return 0u;
		}

		glTexImage2D(current->kind,
		             0,
		             GL_RGBA,
